  ...
  protected static void processLocation(final Context context, final Location location) {
      ...
      if (!usePreviousReading) {
          ...
          if (LocationLibrary.showDebugOutput) 
        	  		  Log.d(LocationLibraryConstants.TAG, TAG + ": Storing location update, lat=" 
        			  + thisLat + " long=" + thisLong + " accuracy=" + thisAccuracy + " time=" 
        	  		  + thisTime + "(" + DateFormat.format("kk:mm.ss, E", thisTime) + ")");
      }
      else {
